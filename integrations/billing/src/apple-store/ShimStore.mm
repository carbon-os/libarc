...
// ObjC++ shim. Owns the StoreBridge instance, converts between:
//   std::string       ↔ NSString
//   std::vector       ↔ NSArray
//   std::function     ↔ ObjC block
//   std::chrono tp    ↔ NSDate
//   C++ value types   ↔ Billing bridge objects
//
// Store::Impl is defined here (incomplete in store.h) so ObjC types
// never leak into the public C++ header.

#import <Foundation/Foundation.h>
#import <AppKit/AppKit.h>
#import "billing_apple-Swift.h"   // generated by CMake from the Swift target
...
// ── Impl definition ───────────────────────────────────────────────────────────
// Defined here where StoreBridge* is a complete type.
...
Store::~Store() = default;  // Impl complete here — unique_ptr destructor works
...
void Store::purchase(const std::string& product_id,
                     ...
                     std::optional<std::string> jws_offer_signature) {
    if (!impl_->bridge) return;
    auto fn = impl_->on_purchase_completed;
    NSString* oid = offer_id             ? to_ns(*offer_id)              : nil;
    NSString* jws = jws_offer_signature  ? to_ns(*jws_offer_signature)   : nil;
